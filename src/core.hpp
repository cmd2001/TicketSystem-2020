...
public:
    ...
    public:
        ...
        // 时间 + 日期
        datentime(const string &t = "", const string &d = "") { // TODO: more readable code?
            if(d != "") {
                int month = (d[1] - '0' - 6);
                switch(month) {
                    case 6: date += 30; // Nov.
        ...
        int operator-(const datentime &o) {
        ...
        inline string get_date() const { // TODO: why December?
            string ret;
            int data = date;
            if(date <= 30) ret =  "06-";
    ...
    public:
        char userName[22];
        char password[32];
        char name[12]; // 汉字 // TODO: a Chinese character encoded in UTF-8 can occupy 4 byte(char) at most, so the size of 12 is not sufficient.
        char mailAddr[25];
        int privilege; // 0-10
    ...
    public:
        ...
        datentime startTime;
        datentime arriving[100]; // 前缀和
        datentime leaving[100];  // 前缀和
...
// *************************************** User Operations **************************************
        ...
        while(i < siz) {
            ...
            if(cmd[i] == "-c") {
                if(!vis[0]) {
                    vis[0] = 1;
                    username = cmd[++i];
                    cur_u = Cur_users.query(username); // TODO: cur_u not found? adding the first user?
            ...
            ++i;
        } // TODO: check whether there are illegal characters in cur_username, username, password, name, mailAddr, etc...
    ...
    int query_profile(const string* cmd, const int siz) {
        ...
        if(this_u.second.privilege < that_u.second.privilege) return 0;
        // TODO: DO NOT COUT in class ticket, return it to main program and handle it with main. Otherwise class ticket cannot be reused by the frontend.
